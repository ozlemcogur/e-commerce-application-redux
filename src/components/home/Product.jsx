...
const Product = ({ product }) => {
  ...
  return (
    <div onClick={() => navigate(`products/${product?.id}`)} className='w-[300px] p-3 mb-5 mx-2 border rounded-md relative cursor-pointer'>
      ...
      <div className='text-center px-2 mt-3 text-xl font-bold'>{product?.title}</div>
    </div>
  )
}
